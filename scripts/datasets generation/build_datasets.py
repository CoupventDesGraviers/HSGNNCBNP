...
def analyse(start, end, wp, shortest_costs, city):    
    ...
    db_file = open('DB_'+city+'_len_'+str(len(wp)) + '.csv', 'a')
...
def main():
    ...
    for root, dirs, files in os.walk('.', topdown=True):
        for name in files:
            ...
            adj_matrix, number_of_nodes, shortest_costs = loadData(city)
            # for problem_size in [5,30,60]:
            for problem_size in [80]:
                if problem_size < number_of_nodes:
                    for _ in range(1000):
